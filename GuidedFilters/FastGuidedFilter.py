...
class FastGuidedFilter(GuidedFilterBase):
	...
	def fast_guided_filter(self, guiding_image_low_resolution, guided_image_low_resolution, guided_image_high_resolution, 
		...
		# N
		N = self.__box_filter__(tf.ones((1, 1, lr_x_shape[2], lr_x_shape[3]), dtype=lr_x.dtype), self.radious)
		# mean_x
		mean_x = self.__box_filter__(lr_x, self.radious) / N
		# mean_y
		mean_y = self.__box_filter__(lr_y, self.radious) / N
		# cov_xy
		cov_xy = self.__box_filter__(lr_x * lr_y, self.radious) / N - mean_x * mean_y
		# var_xself.
		var_x  = self.__box_filter__(lr_x * lr_x, self.radious) / N - mean_x * mean_x
		# A
